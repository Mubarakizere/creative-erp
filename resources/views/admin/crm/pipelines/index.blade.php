                    ...
                    @forelse($pipelines as $pipeline)
                        <tr class="hover:bg-blue-50/40 transition-colors group">
                            ...
                            <td class="px-6 py-5 text-right">
                                <x-action-dropdown>
                                    @can('view', $pipeline)
                                        <x-action-dropdown-item href="{{ route('admin.crm.pipelines.show', $pipeline) }}" icon="view">
                                            View Details
                                        </x-action-dropdown-item>
                                    @endcan

                                    @can('update', $pipeline)
                                        <x-action-dropdown-item href="{{ route('admin.crm.pipelines.edit', $pipeline) }}" icon="edit">
                                            Edit Settings
                                        </x-action-dropdown-item>
                                    @endcan

                                    @can('delete', $pipeline)
                                        <x-action-dropdown-item @click="$dispatch('open-modal', 'delete-pipeline-{{ $pipeline->id }}')" icon="delete" variant="danger">
                                            Delete Pipeline
                                        </x-action-dropdown-item>
                                    @endcan
                                </x-action-dropdown>
