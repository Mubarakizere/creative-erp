                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <x-select name="account_id" label="Account" placeholder="Select Account" :options="$accounts->pluck('name', 'id')->toArray()" :selected="old('account_id', $quotation->account_id)" />
                                    <x-select name="opportunity_id" label="Opportunity" placeholder="Select Opportunity" :options="$opportunities->pluck('name', 'id')->toArray()" :selected="old('opportunity_id', $quotation->opportunity_id)" />
                                    <x-select name="lead_id" label="Lead" placeholder="Select Lead" :options="$leads->mapWithKeys(fn($l) => [$l->id => $l->first_name . ' ' . $l->last_name])->toArray()" :selected="old('lead_id', $quotation->lead_id)" />
                                    <x-select name="contact_id" label="Contact" placeholder="Select Contact" :options="$contacts->mapWithKeys(fn($c) => [$c->id => $c->first_name . ' ' . $c->last_name])->toArray()" :selected="old('contact_id', $quotation->contact_id)" />
                                </div>
