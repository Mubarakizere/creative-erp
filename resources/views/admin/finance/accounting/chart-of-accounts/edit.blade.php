<x-layouts.admin title="Edit Account - Chart of Accounts">
    <x-slot:breadcrumbs>
        @php
            $breadcrumbs = [
                ...
                ['label' => 'Edit ' . $chartOfAccount->code]
            ];
        @endphp
    </x-slot:breadcrumbs>
            ...
            <div>
                <div class="flex items-center gap-2">
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">
                        Edit Account: {{ $chartOfAccount->name }}
                    </h1>
                    <span class="font-mono text-xs font-bold text-blue-600 px-2.5 py-0.5 bg-blue-50 border border-blue-100 rounded-md">
                        {{ $chartOfAccount->code }}
                    </span>
                    @if($chartOfAccount->is_system)
                        <span class="px-2 py-0.5 text-[10px] font-bold bg-slate-100 text-slate-600 border border-slate-200 rounded-md uppercase tracking-wider">
                            System Account
                        </span>
                    @endif
                </div>
                <p class="mt-0.5 text-xs sm:text-sm text-gray-500">Update general ledger account code, classification, and operating status.</p>
            </div>
        </div>
    </div>
    ...
    @if($chartOfAccount->is_system)
        <div class="mb-6 bg-amber-50 border border-amber-200/80 text-amber-800 px-4 py-3 rounded-xl shadow-sm flex items-start gap-3">
            <svg class="w-5 h-5 text-amber-600 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
            </svg>
            <div class="text-xs">
                <span class="font-bold block text-amber-900 mb-0.5">System Account Protection</span>
                This is an core system account automatically managed by Creative ERP for default financial posting workflows (e.g. Accounts Receivable/Payable). Exercise caution when editing.
            </div>
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('admin.finance.accounting.chart-of-accounts.update', $chartOfAccount) }}" class="space-y-6 max-w-4xl">
        @csrf
        @method('PUT')
        ...
        <x-card class="p-6 bg-white border border-gray-200/80 shadow-sm rounded-xl">
                ...
                <div>
                    ...
                    <select name="account_type_id" id="account_type_id" required class="w-full text-xs">
                        ...
                        @foreach($accountTypes as $type)
                            <option value="{{ $type->id }}" {{ old('account_type_id', $chartOfAccount->account_type_id) == $type->id ? 'selected' : '' }}>
            ...
            <div class="mt-6 pt-4 border-t border-gray-100">
                ...
                <textarea name="description" 
                          id="description" 
                          rows="3" 
                          class="w-full px-3.5 py-2 text-xs rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">{{ old('description', $chartOfAccount->description) }}</textarea>
