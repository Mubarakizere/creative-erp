                    ...
                    @forelse($invoices as $invoice)
                        <tr class="hover:bg-blue-50/30 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-right">
                                <x-action-dropdown>
                                    @can('view', $invoice)
                                        <x-action-dropdown-item href="{{ route('admin.finance.invoices.show', $invoice) }}" icon="view">
                                            View Details
                                        </x-action-dropdown-item>
                                    @endcan

                                    @if($invoice->status === 'Draft' && auth()->user()->can('update', $invoice))
                                        <x-action-dropdown-item href="{{ route('admin.finance.invoices.edit', $invoice) }}" icon="edit">
                                            Edit Invoice
                                        </x-action-dropdown-item>

                                        <x-action-dropdown-item @click="$dispatch('open-modal', 'issue-invoice-{{ $invoice->id }}')">
                                            Issue Invoice
                                        </x-action-dropdown-item>
                                    @endif
                                    ...
                                    @if($invoice->balance_due > 0 && !in_array($invoice->status, ['Draft', 'Cancelled']))
                                        @can('create', App\Models\Payment::class)
                                            <x-action-dropdown-item href="{{ route('admin.finance.payments.create', ['invoice_id' => $invoice->id]) }}">
                                                Record Payment
                                            </x-action-dropdown-item>
                                        @endcan
                                    @endif
                                    ...
                                    @can('delete', $invoice)
                                        <x-action-dropdown-item @click="$dispatch('open-modal', 'delete-invoice-{{ $invoice->id }}')" icon="delete" variant="danger">
                                            Delete Invoice
                                        </x-action-dropdown-item>
                                    @endcan
                                </x-action-dropdown>
