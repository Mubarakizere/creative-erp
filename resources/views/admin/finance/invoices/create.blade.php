<x-layouts.admin title="Create Invoice">
    ...
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Create New Invoice</h1>
        <p class="mt-1 text-sm text-gray-500 font-medium">Generate a new invoice for a client.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.finance.invoices.store') }}" 
        ...
        @if(isset($quotation))
            ...
            <div class="bg-blue-50/50 text-blue-800 p-4 rounded-xl border border-blue-100 flex items-center mb-6">
                <div class="bg-blue-100 text-blue-600 p-2 rounded-lg shrink-0 mr-3">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>
                <div>
                    <span class="font-medium text-blue-900">Creating invoice from Quotation:</span> <span class="font-bold">{{ $quotation->quotation_number }}</span>
                </div>
            </div>
        @endif
            ...
            <div class="bg-white rounded-2xl border border-gray-200/60 shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="2" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors">{{ old('notes', $quotation->notes ?? '') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200/60 shadow-sm px-6 py-4 flex items-center justify-end gap-3 mb-8">
            <a href="{{ route('admin.finance.invoices.index') }}" class="inline-flex justify-center px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">Cancel</a>
