<x-layouts.admin title="Edit Expertise Card">
    <x-slot:breadcrumbs>
        @php
            $breadcrumbs = [
                ['label' => 'Website CMS'],
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Expertise Card</h1>
    ...
    <form method="POST" action="{{ route('admin.expertise-cards.update', $expertiseCard) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            <x-card>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-6">
                        <x-input name="title" label="Title" value="{{ old('title', $expertiseCard->title) }}" required />
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Description <span class="text-red-500">*</span></label>
                            <textarea name="description" rows="5" class="block w-full rounded-lg border border-gray-300 shadow-sm text-sm py-2.5 px-3 focus:ring-blue-500 focus:border-blue-500 resize-none" required>{{ old('description', $expertiseCard->description) }}</textarea>
                            @error('description') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <x-input name="sort_order" type="number" label="Sort Order" value="{{ old('sort_order', $expertiseCard->sort_order) }}" />
                            
                            <div class="flex flex-col justify-end pb-3">
                                <label class="inline-flex items-center cursor-pointer">
                                    <input type="checkbox" name="is_active" value="1" class="sr-only peer" @checked(old('is_active', $expertiseCard->is_active))>
                    ...
                    <div class="bg-gray-50 p-5 rounded-xl border border-gray-200">
                        ...
                        @if($expertiseCard->image)
                            <div class="mb-5 rounded-lg overflow-hidden border border-gray-200 bg-white aspect-video relative group">
                                <img src="{{ Str::startsWith($expertiseCard->image, 'http') ? $expertiseCard->image : asset($expertiseCard->image) }}" class="w-full h-full object-cover" />
                            </div>
                        @endif
                        ...
                        <div class="space-y-5">
                            ...
                            <x-input name="image_url" type="url" label="Paste New Image URL" value="{{ Str::startsWith(old('image_url', $expertiseCard->image) ?? '', 'http') ? old('image_url', $expertiseCard->image) : '' }}" placeholder="https://images.unsplash.com/..." />
                        </div>
                    </div>
                </div>
            </x-card>
