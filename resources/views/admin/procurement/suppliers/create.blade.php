<x-layouts.admin title="Add Supplier">
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Add New Supplier</h1>
                <p class="mt-1 text-sm text-slate-500 font-medium">Create a new vendor profile in the procurement database.</p>
            </div>
        </div>

        <form action="{{ route('admin.procurement.suppliers.store') }}" method="POST" id="supplier-form">
            @csrf
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden mb-6">
                <div class="bg-slate-50/80 border-b border-slate-200/80 px-6 py-4">
                    ...
                    <p class="mt-0.5 text-xs text-slate-500">Provide basic contact and classification details for this supplier.</p>
                </div>
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="code" id="code" value="{{ old('code') }}" required placeholder="e.g. SUP-2026-001" class="block w-full rounded-xl border-slate-300 shadow-xs focus:border-indigo-500 focus:ring-indigo-500 text-xs py-2.5 transition-colors">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="e.g. Acme Supplies Ltd" class="block w-full rounded-xl border-slate-300 shadow-xs focus:border-indigo-500 focus:ring-indigo-500 text-xs py-2.5 transition-colors">
                        ...
                        <div>
                            ...
                            <select name="supplier_category_id" id="supplier_category_id" class="block w-full rounded-xl border-slate-300 shadow-xs focus:border-indigo-500 focus:ring-indigo-500 text-xs py-2.5 transition-colors">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('supplier_category_id') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email') }}" placeholder="supplier@example.com" class="block w-full rounded-xl border-slate-300 shadow-xs focus:border-indigo-500 focus:ring-indigo-500 text-xs py-2.5 transition-colors">
                        ...
                        <div>
                            ...
                            <input type="text" name="phone" id="phone" value="{{ old('phone') }}" placeholder="+250 788 000 000" class="block w-full rounded-xl border-slate-300 shadow-xs focus:border-indigo-500 focus:ring-indigo-500 text-xs py-2.5 transition-colors">
                        ...
                        <div class="sm:col-span-2 pt-2">
                            <div class="bg-amber-50/60 border border-amber-200/80 rounded-xl p-4 flex items-start gap-3">
                                <div class="flex h-5 items-center mt-0.5">
                                    <input type="checkbox" name="is_preferred" id="is_preferred" value="1" {{ old('is_preferred') ? 'checked' : '' }} class="h-4 w-4 rounded border-amber-300 text-amber-600 focus:ring-amber-500 transition-colors">
                                </div>
                                <div class="text-xs">
    ...
    <div class="text-center py-16 bg-white rounded-2xl border border-slate-200/80 shadow-xs p-8">
        ...
        <p class="text-sm text-slate-500 font-medium">You do not have permission to add suppliers.</p>
        <div class="mt-6">
