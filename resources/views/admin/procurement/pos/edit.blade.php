<x-layouts.admin title="Edit Purchase Order">
    <x-slot:breadcrumbs>
        @php
            $breadcrumbs = [
                ...
                ['label' => 'Purchase Orders', 'url' => route('admin.procurement.pos.index')],
                ['label' => $po->code, 'url' => route('admin.procurement.pos.show', $po->id)],
                ['label' => 'Edit'],
            ];
        @endphp
    </x-slot:breadcrumbs>
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Purchase Order: {{ $po->code }}</h1>
            <p class="mt-1 text-sm text-gray-500 font-medium">Update the purchase order and its items.</p>
        </div>
    </div>

    <div class="mt-6" x-data="{ items: {{ count($po->items) > 0 ? Js::from($po->items->map(fn($i) => ['product_id' => $i->product_id, 'quantity' => $i->quantity, 'unit_price' => $i->unit_price ?? 0])) : '[{ product_id: \'\', quantity: 1, unit_price: 0 }]' }} }">
        <form action="{{ route('admin.procurement.pos.update', $po->id) }}" method="POST" id="po-form">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl border border-gray-200/60 shadow-sm overflow-hidden mb-6">
                <div class="bg-gray-50/50 border-b border-gray-100 px-6 py-4">
                    <h3 class="text-lg font-bold text-gray-900 tracking-tight">Order Details</h3>
                </div>
                <div class="p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div class="col-span-1 lg:col-span-2">
                            <label for="code" class="block text-sm font-medium text-gray-700 mb-1">Order Number <span class="text-red-500">*</span></label>
                            <input type="text" name="code" id="code" value="{{ old('code', $po->code) }}" required class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors">
                        ...
                        <div class="col-span-1 sm:col-span-1 lg:col-span-2">
                            ...
                            <input type="date" name="order_date" id="order_date" value="{{ old('order_date', $po->order_date) }}" required class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors">
                        ...
                        <div class="col-span-1 sm:col-span-1 lg:col-span-2">
                            ...
                            <select id="status" name="status" required class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors bg-white">
                                ...
                                <option value="pending" {{ old('status', $po->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="approved" {{ old('status', $po->status) == 'approved' ? 'selected' : '' }}>Approved</option>
                                <option value="partially_received" {{ old('status', $po->status) == 'partially_received' ? 'selected' : '' }}>Partially Received</option>
                                <option value="completed" {{ old('status', $po->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                                <option value="cancelled" {{ old('status', $po->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                            </select>
            ...
            <div class="bg-white rounded-2xl border border-gray-200/60 shadow-sm overflow-hidden mb-6">
                <div class="bg-gray-50/50 border-b border-gray-100 px-6 py-4 flex justify-between items-center">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 font-medium">Add products to this purchase order.</p>
                    </div>
                    <button type="button" @click="items.push({ product_id: '', quantity: 1, unit_price: 0 })" class="inline-flex items-center px-4 py-2 text-sm font-medium text-blue-700 bg-blue-100 rounded-xl hover:bg-blue-200 transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <table class="min-w-full divide-y divide-gray-200/60">
                        <thead class="bg-gray-50/30">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100 w-48">Unit Price <span class="text-red-500">*</span></th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100 w-48">Total</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            <template x-for="(item, index) in items" :key="index">
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm font-medium text-gray-900" x-text="((parseFloat(item.quantity) || 0) * (parseFloat(item.unit_price) || 0)).toFixed(2)"></span>
                                    </td>
    ...
    <div class="text-center py-16 bg-white rounded-2xl border border-gray-200/60 shadow-sm">
        ...
        <p class="text-sm text-gray-500 font-medium">You do not have permission to edit this purchase order.</p>
        <div class="mt-6">
            <a href="{{ route('admin.procurement.pos.show', $po->id) }}" class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-sm transition-all">Return to Order</a>
        </div>
    </div>
    @endcan
</x-layouts.admin>
