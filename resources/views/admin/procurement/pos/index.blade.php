<x-layouts.admin title="Purchase Orders">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 font-medium">Manage and track your purchase orders to suppliers.</p>
        </div>
        <div class="flex items-center gap-3">
            @can('create', App\Models\PurchaseOrder::class)
                <a href="{{ route('admin.procurement.pos.create') }}" class="inline-flex items-center gap-2 px-5 py-2 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-sm transition-all hover:shadow-md focus:ring-2 focus:ring-blue-500 focus:outline-none shrink-0">
    ...
    <div class="bg-white p-5 rounded-2xl border border-gray-200/60 shadow-sm mb-6">
        <form method="GET" action="{{ route('admin.procurement.pos.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                </div>
                <input type="text" name="search" placeholder="Search orders..." value="{{ request('search') }}" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-xl leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-colors shadow-sm min-h-[42px]">
            </div>
            ...
            <div class="flex items-end gap-2 lg:col-span-3">
                ...
                @if(request()->has('search'))
                    <a href="{{ route('admin.procurement.pos.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-900 bg-white border border-transparent hover:border-gray-300 rounded-xl transition-colors shrink-0 min-h-[42px]">
                        Clear
                    </a>
                @endif
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200/60">
                <thead class="bg-gray-50/50">
                    <tr>
                        <th class="px-6 py-4 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100">Order Number</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-blue-50/30 transition-colors group">
                        <td class="px-6 py-4">
                            <span class="text-sm font-semibold text-gray-900">{{ $po->code }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-sm text-gray-600 font-medium">{{ $po->supplier?->name ?? 'N/A' }}</span>
                        </td>
                        <td class="px-6 py-4 hidden md:table-cell">
                            <span class="text-sm text-gray-600">{{ $po->order_date }}</span>
                        </td>
                        <td class="px-6 py-4">
                            @php
                                $statusType = match(strtolower($po->status)) {
                                    'draft' => 'default',
                                    'pending' => 'warning',
                                    'approved' => 'success',
                        ...
                        <td class="px-6 py-4 text-right">
                            <div x-data="{ open: false }" class="relative inline-block text-left">
                                ...
                                <div x-show="open" class="absolute right-0 z-10 mt-2 w-48 rounded-xl bg-white shadow-lg ring-1 ring-black/5 py-1" style="display: none;">
                                    @can('view', $po)
                                        <a href="{{ route('admin.procurement.pos.show', $po->id) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">View</a>
                                    @endcan
                                    @can('update', $po)
                                        <a href="{{ route('admin.procurement.pos.edit', $po->id) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Edit</a>
                                    @endcan
                                    @if(strtolower($po->status) === 'draft')
                                        ...
                                        <form action="{{ route('admin.procurement.pos.approve', $po->id) }}" method="POST" class="block w-full text-left">
                                            @csrf
                                            <button type="submit" class="w-full text-left px-4 py-2 text-sm text-blue-600 hover:bg-gray-50">Approve PO</button>
                                        </form>
                                        @endcan
                                    @elseif(in_array(strtolower($po->status), ['approved', 'partially_received']))
                                        @can('create', App\Models\GoodsReceipt::class)
                                        <a href="{{ route('admin.procurement.receipts.create', ['po_id' => $po->id]) }}" class="block px-4 py-2 text-sm text-green-600 hover:bg-gray-50">Receive Goods</a>
                                        @endcan
                                    @endif
                                </div>
                            </div>
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center">
                            ...
                            <p class="text-sm text-gray-500 font-medium">Create your first purchase order to get started.</p>
                            @can('create', App\Models\PurchaseOrder::class)
                                <a href="{{ route('admin.procurement.pos.create') }}" class="mt-4 inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-sm transition-all focus:ring-2 focus:ring-blue-500 focus:outline-none">
                                    New Purchase Order
                                </a>
                            @endcan
                        </td>
    ...
    <div class="text-center py-16 bg-white rounded-2xl border border-gray-200/60 shadow-sm">
        ...
        <p class="text-sm text-gray-500 font-medium">You do not have permission to view purchase orders.</p>
    </div>
    @endcan
</x-layouts.admin>