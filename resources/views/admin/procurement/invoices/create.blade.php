<x-layouts.admin title="Record Purchase Invoice">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <a href="{{ route('admin.procurement.invoices.index') }}" class="inline-flex items-center text-sm font-medium text-blue-600 hover:text-blue-800 mb-2 transition-colors">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                Back to Invoices
            </a>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Record Purchase Invoice</h1>
            <p class="mt-1 text-sm text-gray-500 font-medium">Record a new invoice received from a supplier.</p>
        </div>
    </div>

    @php
        // Prepare Alpine JS data depending on whether we are loading from a PO or standalone
        $alpineData = "{ items: [] }";
        if (isset($po) && $po->items) {
            $mappedItems = $po->items->map(function($item) {
                return [
            ...
            $alpineData = "{ items: [{ product_id: '', quantity: 1, unit_price: 0, tax: 0, discount: 0, product_name: '' }] }";
        }
    @endphp

    <div class="mt-6" x-data="{{ $alpineData }}">
        <form action="{{ route('admin.procurement.invoices.store') }}" method="POST" id="invoice-form">
            @csrf
            @if(isset($po))
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div class="col-span-1 lg:col-span-2">
                            ...
                            <input type="text" name="invoice_number" id="invoice_number" value="{{ old('invoice_number') }}" required class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors">
                        ...
                        <div class="col-span-1 lg:col-span-2">
                            ...
                            @if(isset($po))
                                ...
                                <input type="text" value="{{ $po->supplier?->name }}" readonly class="block w-full rounded-xl border-gray-200 bg-gray-50 text-gray-500 sm:text-sm">
                            @else
                        ...
                        <div class="col-span-1 sm:col-span-1 lg:col-span-2">
                            ...
                            <input type="date" name="invoice_date" id="invoice_date" value="{{ old('invoice_date', date('Y-m-d')) }}" required class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors">
                        ...
                        <div class="col-span-1 sm:col-span-1 lg:col-span-2">
                            ...
                            <input type="date" name="due_date" id="due_date" value="{{ old('due_date', date('Y-m-d', strtotime('+30 days'))) }}" required class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors">
            ...
            <div class="bg-white rounded-2xl border border-gray-200/60 shadow-sm overflow-hidden mb-6">
                <div class="bg-gray-50/50 border-b border-gray-100 px-6 py-4 flex justify-between items-center">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 font-medium">Verify or add invoice items and pricing.</p>
                    </div>
                    @if(!isset($po))
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            <template x-for="(item, index) in items" :key="index">
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <input type="number" step="0.01" min="0.01" x-model="item.quantity" :name="'items[' + index + '][quantity]'" required class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" {{ isset($po) ? 'readonly' : '' }}>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
            ...
            <div class="bg-white rounded-2xl border border-gray-200/60 shadow-sm overflow-hidden px-6 py-4 flex items-center justify-end gap-3 mb-8">
                <a href="{{ route('admin.procurement.invoices.index') }}" class="inline-flex items-center justify-center px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">Cancel</a>
    ...
    <div class="text-center py-16 bg-white rounded-2xl border border-gray-200/60 shadow-sm">
        ...
        <p class="text-sm text-gray-500 font-medium">You do not have permission to record purchase invoices.</p>
        <div class="mt-6">
            <a href="{{ route('admin.procurement.invoices.index') }}" class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-sm transition-all">Return to Invoices</a>
        </div>
    </div>
    @endcan
</x-layouts.admin>