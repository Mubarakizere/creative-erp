<x-layouts.admin title="Purchase Invoices">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Purchase Invoices</h1>
            <p class="mt-1 text-sm text-gray-500 font-medium">Manage incoming invoices from your suppliers.</p>
        </div>
        <div class="flex items-center gap-3">
            @can('create', App\Models\PurchaseInvoice::class)
                <a href="{{ route('admin.procurement.invoices.create') }}" class="inline-flex items-center gap-2 px-5 py-2 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-sm transition-all hover:shadow-md focus:ring-2 focus:ring-blue-500 focus:outline-none shrink-0">
    ...
    <div class="bg-white p-5 rounded-2xl border border-gray-200/60 shadow-sm mb-6">
        <form method="GET" action="{{ route('admin.procurement.invoices.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                </div>
                <input type="text" name="search" placeholder="Search invoices..." value="{{ request('search') }}" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-xl leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-colors shadow-sm min-h-[42px]">
            </div>
            ...
            <div class="flex items-end gap-2 lg:col-span-3">
                ...
                @if(request()->has('search'))
                    <a href="{{ route('admin.procurement.invoices.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-900 bg-white border border-transparent hover:border-gray-300 rounded-xl transition-colors shrink-0 min-h-[42px]">
                        Clear
                    </a>
                @endif
            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200/60">
                <thead class="bg-gray-50/50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100 hidden md:table-cell">Date</th>
                        <th class="px-6 py-4 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100">Total</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-blue-50/30 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <span class="text-sm text-gray-600 font-medium">{{ $invoice->supplier->name ?? 'N/A' }}</span>
                        </td>
                        <td class="px-6 py-4 hidden md:table-cell">
                            <span class="text-sm text-gray-600">{{ $invoice->invoice_date->format('M d, Y') }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-sm font-semibold text-gray-900">RWF {{ number_format($invoice->grand_total, 2) }}</span>
                        </td>
                        <td class="px-6 py-4">
                            @php
                                $statusType = match(strtolower($invoice->status)) {
                                    'draft' => 'default',
                        ...
                        <td class="px-6 py-4 text-right">
                            <div x-data="{ open: false }" class="relative inline-block text-left">
                                ...
                                <div x-show="open" class="absolute right-0 z-10 mt-2 w-48 rounded-xl bg-white shadow-lg ring-1 ring-black/5 py-1" style="display: none;">
                                    @can('view', $invoice)
                                        <a href="{{ route('admin.procurement.invoices.show', $invoice->id) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">View Details</a>
                                    @endcan
                                    @can('update', $invoice)
                                        <a href="{{ route('admin.procurement.invoices.edit', $invoice->id) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Edit Invoice</a>
                                    @endcan
                                    @if(in_array(strtolower($invoice->status), ['unpaid', 'partially_paid']))
                                        @can('create', App\Models\SupplierPayment::class)
                                            <a href="{{ route('admin.procurement.payments.create', ['invoice_id' => $invoice->id]) }}" class="block px-4 py-2 text-sm text-green-600 hover:bg-gray-50">Record Payment</a>
                                        @endcan
                                    @endif
                                </div>
                            </div>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-16 text-center">
                            ...
                            <p class="text-sm text-gray-500 font-medium">Record your first purchase invoice from a supplier.</p>
                            @can('create', App\Models\PurchaseInvoice::class)
                                <a href="{{ route('admin.procurement.invoices.create') }}" class="mt-4 inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-sm transition-all focus:ring-2 focus:ring-blue-500 focus:outline-none">
                                    Record Invoice
                                </a>
                            @endcan
                        </td>
    ...
    <div class="text-center py-16 bg-white rounded-2xl border border-gray-200/60 shadow-sm">
        ...
        <p class="text-sm text-gray-500 font-medium">You do not have permission to view purchase invoices.</p>
    </div>
    @endcan
</x-layouts.admin>