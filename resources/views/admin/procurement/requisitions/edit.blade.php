<x-layouts.admin title="Edit Requisition">
    <x-slot:breadcrumbs>
        @php
            $breadcrumbs = [
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <a href="{{ route('admin.procurement.requisitions.show', $requisition->id) }}" class="inline-flex items-center text-sm font-medium text-blue-600 hover:text-blue-800 mb-2 transition-colors">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                Back to Requisition
            </a>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Purchase Requisition: {{ $requisition->code }}</h1>
            <p class="mt-1 text-sm text-gray-500 font-medium">Update purchase requisition and items.</p>
        </div>
    </div>

    <div class="mt-6" x-data="{ items: {{ count($requisition->items) > 0 ? Js::from($requisition->items->map(fn($i) => ['product_id' => $i->product_id, 'quantity' => $i->quantity])) : '[{ product_id: \'\', quantity: 1 }]' }} }">
        <form action="{{ route('admin.procurement.requisitions.update', $requisition->id) }}" method="POST" id="requisition-form">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="code" id="code" value="{{ old('code', $requisition->code) }}" required class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors">
            ...
            <div class="bg-white rounded-2xl border border-gray-200/60 shadow-sm overflow-hidden px-6 py-4 flex items-center justify-end gap-3 mb-8">
                <a href="{{ route('admin.procurement.requisitions.show', $requisition->id) }}" class="inline-flex items-center justify-center px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">Cancel</a>
    ...
    <div class="text-center py-16 bg-white rounded-2xl border border-gray-200/60 shadow-sm">
        ...
        <p class="text-sm text-gray-500 font-medium">You do not have permission to edit this requisition.</p>
        <div class="mt-6">
            <a href="{{ route('admin.procurement.requisitions.show', $requisition->id) }}" class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-sm transition-all">Return to Requisition</a>
        </div>
    </div>
    @endcan
</x-layouts.admin>
