<x-layouts.admin title="Edit Requisition: {{ $requisition->code }}">
    <x-slot:breadcrumbs>
        @php
            $breadcrumbs = [
                ...
                ['label' => $requisition->code, 'url' => route('admin.procurement.requisitions.show', $requisition->id)],
                ['label' => 'Edit'],
            ];
        @endphp
    </x-slot:breadcrumbs>
    ...
    @can('update', $requisition)
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <a href="{{ route('admin.procurement.requisitions.show', $requisition->id) }}" class="inline-flex items-center text-sm font-semibold text-indigo-600 hover:text-indigo-800 mb-2 transition-colors">
                        <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                        Back to Requisition Details
                    </a>
                    <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Edit Purchase Requisition: {{ $requisition->code }}</h1>
                    <p class="mt-1 text-sm text-slate-500 font-medium">Update destination project, company assignment, timeline, or requested items.</p>
                </div>
            </div>
            ...
            <form action="{{ route('admin.procurement.requisitions.update', $requisition->id) }}" method="POST" id="requisition-form"
                  x-data="purchaseRequisitionForm({
                      projectsData: {{ \Illuminate\Support\Js::from($projectsData) }},
                      initialProjectId: '{{ old('project_id', $requisition->project_id ?? '') }}',
                      initialCompanyId: '{{ old('company_id', $requisition->company_id ?? '') }}',
                      initialItems: {{ \Illuminate\Support\Js::from(old('items', $requisition->items->map(fn($i) => ['product_id' => $i->product_id, 'quantity' => (float)$i->quantity, 'description' => $i->description ?? '']))) }},
                ...
                <x-card class="mb-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6">
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-2">
                            ...
                            <input type="text" name="code" id="code" value="{{ old('code', $requisition->code) }}" required
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-2">
                            <label for="status" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 mb-1.5">
                                Requisition Status <span class="text-rose-500 font-bold ml-0.5">*</span>
                            </label>
                            <div class="relative group">
                                <select id="status" name="status" required
                                        class="block w-full rounded-xl text-sm transition-all duration-200 border appearance-none border-slate-300 bg-slate-50/50 hover:bg-white hover:border-slate-400 text-slate-900 focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-500/10 pl-3.5 pr-10 py-2.5 min-h-[42px] shadow-xs cursor-pointer font-semibold">
                                    <option value="draft" {{ old('status', $requisition->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                                    <option value="submitted" {{ old('status', $requisition->status) == 'submitted' ? 'selected' : '' }}>Submit for Approval</option>
                                    @if(in_array($requisition->status, ['approved', 'rejected', 'cancelled']))
                                        <option value="{{ $requisition->status }}" selected>{{ ucfirst($requisition->status) }}</option>
                                    @endif
                                </select>
                                <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center pointer-events-none text-slate-400 group-focus-within:text-indigo-600 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-2">
                            ...
                            <div class="relative group">
                                <select id="priority" name="priority" required
                                        class="block w-full rounded-xl text-sm transition-all duration-200 border appearance-none border-slate-300 bg-slate-50/50 hover:bg-white hover:border-slate-400 text-slate-900 focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-500/10 pl-3.5 pr-10 py-2.5 min-h-[42px] shadow-xs cursor-pointer font-semibold">
                                    <option value="normal" {{ old('priority', strtolower($requisition->priority ?? 'normal')) == 'normal' ? 'selected' : '' }}>Normal Priority</option>
                                    <option value="high" {{ old('priority', strtolower($requisition->priority ?? 'normal')) == 'high' ? 'selected' : '' }}>High Priority</option>
                                    <option value="urgent" {{ old('priority', strtolower($requisition->priority ?? 'normal')) == 'urgent' ? 'selected' : '' }}>Urgent Priority</option>
                                    <option value="low" {{ old('priority', strtolower($requisition->priority ?? 'normal')) == 'low' ? 'selected' : '' }}>Low Priority</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center pointer-events-none text-slate-400 group-focus-within:text-indigo-600 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-2">
                            ...
                            <input type="date" name="required_date" id="required_date" value="{{ old('required_date', $requisition->required_date ? $requisition->required_date->format('Y-m-d') : '') }}"
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-6">
                            ...
                            <textarea name="notes" id="notes" rows="3" placeholder="Provide background, justification, or special purchasing constraints..."
                                      class="block w-full rounded-xl text-sm transition-all duration-200 border border-slate-300 bg-slate-50/50 hover:bg-white text-slate-900 focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-500/10 p-3.5 shadow-xs font-medium">{{ old('notes', $requisition->notes) }}</textarea>
                    ...
                    <x-card>
                        ...
                        <x-slot:footer>
                            <div class="flex items-center justify-end gap-3 w-full">
                                <a href="{{ route('admin.procurement.requisitions.show', $requisition->id) }}"
    ...
    @else
        <div class="text-center py-16 bg-white rounded-2xl border border-slate-200/80 shadow-xs">
            ...
            <p class="text-sm text-slate-500 font-medium">You do not have permission to edit this purchase requisition.</p>
            <div class="mt-6">
                <a href="{{ route('admin.procurement.requisitions.show', $requisition->id) }}" class="px-5 py-2.5 text-sm font-bold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 shadow-xs transition-all">Return to Requisition</a>
            </div>
        </div>
    @endcan
