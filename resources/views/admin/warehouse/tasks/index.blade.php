<x-layouts.admin title="Warehouse Tasks">
    <x-slot:breadcrumbs>
        @php
            $breadcrumbs = [
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Manage Warehouse Tasks.</p>
        </div>
        @can('create', App\Models\WarehouseTask::class)
            <x-button type="primary" href="{{ route('admin.warehouse.tasks.create') }}">
                    ...
                    @forelse($items as $item)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="py-3 px-4 text-right">
                                <x-action-dropdown>
                                    @can('view', $item)
                                        <x-action-dropdown-item href="{{ route('admin.warehouse.tasks.show', $item) }}" icon="view">
                                            View Details
                                        </x-action-dropdown-item>
                                    @endcan

                                    @can('update', $item)
                                        <x-action-dropdown-item href="{{ route('admin.warehouse.tasks.edit', $item) }}" icon="edit">
                                            Edit Task
                                        </x-action-dropdown-item>
                                    @endcan

                                    @can('delete', $item)
                                        <form method="POST" action="{{ route('admin.warehouse.tasks.destroy', $item) }}" id="delete-task-form-{{ $item->id }}">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                        <x-action-dropdown-item onclick="document.getElementById('delete-task-form-{{ $item->id }}').submit()" icon="delete" variant="danger">
                                            Delete Task
                                        </x-action-dropdown-item>
                                    @endcan
                                </x-action-dropdown>
