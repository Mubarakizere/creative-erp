                    ...
                    @forelse($items as $item)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="py-3 px-4 text-right">
                                <x-action-dropdown>
                                    @can('view', $item)
                                        <x-action-dropdown-item href="{{ route('admin.warehouse.packings.show', $item) }}" icon="view">
                                            View Details
                                        </x-action-dropdown-item>
                                    @endcan

                                    @can('update', $item)
                                        <x-action-dropdown-item href="{{ route('admin.warehouse.packings.edit', $item) }}" icon="edit">
                                            Edit Packing
                                        </x-action-dropdown-item>
                                    @endcan

                                    @can('delete', $item)
                                        <form method="POST" action="{{ route('admin.warehouse.packings.destroy', $item) }}" id="delete-packing-form-{{ $item->id }}">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                        <x-action-dropdown-item onclick="document.getElementById('delete-packing-form-{{ $item->id }}').submit()" icon="delete" variant="danger">
                                            Delete Packing
                                        </x-action-dropdown-item>
                                    @endcan
                                </x-action-dropdown>
