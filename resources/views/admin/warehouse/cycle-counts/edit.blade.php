<x-layouts.admin title="Edit Cycle Counts">
    <x-slot:breadcrumbs>
        @php
            $breadcrumbs = [
    ...
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900">Edit Cycle Counts</h1>
    </div>

    <x-card>
        <form action="{{ route('admin.warehouse.cycle-counts.update', $item) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
