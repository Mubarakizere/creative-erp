        ...
        @forelse($branches as $branch)
            <tr @class(['bg-red-50/30' => $branch->trashed()])>
                ...
                <td class="px-4 py-3 hidden lg:table-cell">
                    <span class="text-sm text-gray-600">{{ $branch->manager_name ?? '—' }}</span>
                </td>
                ...
                <td class="px-4 py-3 text-right">
                    <div x-data="{ open: false }" class="relative inline-block text-left">
                        ...
                        <div x-show="open"
                            ...
                            @can('view', $branch)
                                <a href="{{ route('admin.branches.show', $branch) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                            ...
                            @if(!$branch->trashed())
                                @can('update', $branch)
                                    <a href="{{ route('admin.branches.edit', $branch) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                                        <svg class="w-4 h-4 mr-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @can('delete', $branch)
                                    <button @click="open = false; $dispatch('open-modal', 'delete-branch-{{ $branch->id }}')"
                            ...
                            @else
                                @can('restore', $branch)
                                    <form method="POST" action="{{ route('admin.branches.restore', $branch) }}">
                                        @csrf
                                        @method('PATCH')
