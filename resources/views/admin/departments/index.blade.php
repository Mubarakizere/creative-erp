        ...
        @forelse($departments as $department)
            <tr @class(['bg-red-50/30' => $department->trashed()])>
                ...
                <td class="px-4 py-3 text-right">
                    <x-action-dropdown>
                        @can('view', $department)
                            <x-action-dropdown-item href="{{ route('admin.departments.show', $department) }}" icon="view">
                                View Department
                        ...
                        @if(!$department->trashed())
                            @can('update', $department)
                                <x-action-dropdown-item href="{{ route('admin.departments.edit', $department) }}" icon="edit">
                                    Edit Department
                                </x-action-dropdown-item>
                            @endcan
                            ...
                            @if($department->status !== 'active')
                                @can('activate', $department)
                                    <form method="POST" action="{{ route('admin.departments.activate', $department) }}" id="activate-department-form-{{ $department->id }}">
                                        @csrf
                                        @method('PATCH')
                                    </form>
                                    <x-action-dropdown-item onclick="document.getElementById('activate-department-form-{{ $department->id }}').submit()">
                                        Activate Department
                                    </x-action-dropdown-item>
                                @endcan
                            @endif
                            ...
                            @if($department->status === 'active')
                                @can('deactivate', $department)
                                    <form method="POST" action="{{ route('admin.departments.deactivate', $department) }}" id="deactivate-department-form-{{ $department->id }}">
                                        @csrf
                                        @method('PATCH')
                                    </form>
                                    <x-action-dropdown-item onclick="document.getElementById('deactivate-department-form-{{ $department->id }}').submit()">
                            ...
                            @can('delete', $department)
                                <x-action-dropdown-item @click="$dispatch('open-modal', 'delete-department-{{ $department->id }}')" icon="delete" variant="danger">
                                    Delete Department
                                </x-action-dropdown-item>
                            @endcan
                        @else
                            @can('restore', $department)
                                <form method="POST" action="{{ route('admin.departments.restore', $department) }}" id="restore-department-form-{{ $department->id }}">
                                    @csrf
                                    @method('PATCH')
                                </form>
                                <x-action-dropdown-item onclick="document.getElementById('restore-department-form-{{ $department->id }}').submit()">
                                    Restore Department
                                </x-action-dropdown-item>
                            @endcan
                        @endif
