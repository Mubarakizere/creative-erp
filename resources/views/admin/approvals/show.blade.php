    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div>
            @if($approval->status === 'Pending Approval')
                ...
                <x-card class="mb-6 border-red-200" x-data>
                    ...
                    <button type="button" @click="$dispatch('open-modal', 'cancel-approval-modal')" class="w-full py-2 border border-red-300 text-red-600 rounded-lg hover:bg-red-50 font-medium transition-colors">Cancel Request</button>

                    <x-modal id="cancel-approval-modal" maxWidth="md">
                        <x-slot:header>Cancel Request</x-slot:header>
                        ...
                        <div class="text-center py-4">
                            ...
                            <h3 class="text-lg font-bold text-gray-900 mb-2">Cancel Request #{{ $approval->id }}?</h3>
                            <p class="text-sm text-gray-500 font-medium">Are you sure you want to cancel this request? Approvers will be notified.</p>
                        </div>

                        <x-slot:footer>
                            <button type="button" @click="show = false" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">Keep Request</button>
                            <form action="{{ route('admin.approvals.action', $approval) }}" method="POST" class="inline">
                                @csrf
                                <input type="hidden" name="comment" value="Cancelled by requester">
                                <button type="submit" name="action" value="cancel" class="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-xl hover:bg-red-700 transition-colors shadow-sm">Cancel Request</button>
                            </form>
                        </x-slot:footer>
                    </x-modal>
                </x-card>
