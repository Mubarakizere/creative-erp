<x-layouts.admin title="Edit Milestone">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <a href="{{ route('admin.milestones.show', $milestone) }}" class="inline-flex items-center text-sm font-medium text-blue-600 hover:text-blue-800 mb-2 transition-colors">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                Back to Milestone
            </a>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Milestone</h1>
            <p class="mt-1 text-sm text-gray-500 font-medium">Update milestone details.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.milestones.update', $milestone) }}" id="milestone-form">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-gray-200/60 shadow-sm overflow-hidden mb-6">
            ...
            <div class="px-6 py-4 bg-gray-50/50 border-t border-gray-100 flex items-center justify-end gap-3">
                <a href="{{ route('admin.milestones.show', $milestone) }}" class="inline-flex items-center justify-center px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">
                    Cancel
                </a>
    ...
    <div class="text-center py-16 bg-white rounded-2xl border border-gray-200/60 shadow-sm">
        ...
        <p class="text-sm text-gray-500 font-medium">You do not have permission to edit this milestone.</p>
        <div class="mt-6">
            <a href="{{ route('admin.milestones.show', $milestone) }}" class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-sm transition-all">Return to Milestone</a>
        </div>
    </div>
    @endcan
</x-layouts.admin>
