                                ...
                                <x-action-dropdown>
                                    ...
                                    @can('delete', $type)
                                        @if($type->chart_of_accounts_count === 0)
                                            <form action="{{ route('admin.account-types.destroy', $type) }}" method="POST" id="delete-type-form-{{ $type->id }}" class="hidden">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                            <x-action-dropdown-item type="button" danger onclick="if(confirm('Are you sure you want to delete this account type?')) document.getElementById('delete-type-form-{{ $type->id }}').submit()">
                                                <x-slot:icon>
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
