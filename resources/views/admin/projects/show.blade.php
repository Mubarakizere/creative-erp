<x-layouts.admin title="Project Profile">
    ...
    <div x-data="{ activeTab: '{{ session('activeTab', 'overview') }}' }" class="mb-8">
        ...
        <div class="mt-6">
            ...
            {{-- Expenses & Salary Tab --}}
            <div x-show="activeTab === 'expenses'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" style="display: none;" x-cloak>
                @include('admin.projects.partials.expenses_tab', ['project' => $project, 'financialSummary' => $financialSummary, 'teamUsers' => $teamUsers])
            </div>

            {{-- Profit & Loss (P&L) Tab --}}
            <div x-show="activeTab === 'pnl'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" style="display: none;" x-cloak>
                @include('admin.projects.partials.pnl_tab', ['project' => $project, 'financialSummary' => $financialSummary])
            </div>
