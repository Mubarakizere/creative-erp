<x-layouts.admin title="Material Requests">
    ...
    <div class="bg-white p-5 rounded-2xl border border-gray-200/60 shadow-sm mb-6">
        <form method="GET" action="{{ route('admin.material-requests.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="relative">
            ...
            <div class="relative">
                <select name="status" class="block w-full rounded-xl border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm min-h-[42px] py-2 pl-3 pr-10">
            ...
            <div class="flex items-end gap-2 lg:col-span-2">
                ...
                @if(request()->hasAny(['search', 'status']))
                    <a href="{{ route('admin.material-requests.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-900 bg-white border border-transparent hover:border-gray-300 rounded-xl transition-colors shrink-0 min-h-[42px]">
                        Clear
                    </a>
                @endif
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200/60">
                <thead class="bg-gray-50/50">
                    <tr>
                        <th class="px-6 py-4 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100">Request Number</th>
                        ...
                        <th class="px-6 py-4 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100 hidden md:table-cell">Requested By</th>
                        ...
                        <th class="px-6 py-4 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100 hidden lg:table-cell">Procurement</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-blue-50/30 transition-colors group">
                        <td class="px-6 py-4">
                            <a href="{{ route('admin.material-requests.show', $request) }}" class="text-sm font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">
                        ...
                        <td class="px-6 py-4 hidden lg:table-cell">
                            @if($request->purchaseRequisition)
                                <a href="{{ route('admin.procurement.requisitions.show', $request->purchaseRequisition) }}" class="text-sm text-blue-600 hover:text-blue-800 font-medium transition-colors">
                                    {{ $request->purchaseRequisition->code }}
                                </a>
                                <span class="ml-1 text-xs text-gray-500 capitalize font-medium">({{ $request->purchaseRequisition->status }})</span>
                            @else
                        ...
                        <td class="px-6 py-4 text-right">
                            <div x-data="{ open: false }" class="relative inline-block text-left">
                                ...
                                <div x-show="open" class="absolute right-0 z-10 mt-2 w-48 rounded-xl bg-white shadow-lg ring-1 ring-black/5 py-1" style="display: none;">
                                    @can('view', $request)
                                        <a href="{{ route('admin.material-requests.show', $request) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">View</a>
                                    @endcan
                                    @can('update', $request)
                                        <a href="{{ route('admin.material-requests.edit', $request) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Edit</a>
                                    @endcan
                                </div>
                            </div>
                        </td>
