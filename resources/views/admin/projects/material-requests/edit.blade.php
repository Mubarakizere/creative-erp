<x-layouts.admin title="Edit Material Request">
    <x-slot:breadcrumbs>
        @php
            $breadcrumbs = [
                ...
                ['label' => $materialRequest->request_number, 'url' => route('admin.material-requests.show', $materialRequest)],
                ['label' => 'Edit'],
            ];
        @endphp
    </x-slot:breadcrumbs>
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <a href="{{ route('admin.material-requests.show', $materialRequest) }}" class="inline-flex items-center text-sm font-medium text-blue-600 hover:text-blue-800 mb-2 transition-colors">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                Back to Request
            </a>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Material Request: {{ $materialRequest->request_number }}</h1>
            <p class="mt-1 text-sm text-gray-500 font-medium">Update information about this material request.</p>
        </div>
    </div>
    ...
    <div class="mt-6">
        <form action="{{ route('admin.material-requests.update', $materialRequest) }}" method="POST" id="material-request-form">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6">
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-3">
                            ...
                            <select id="task_id" name="task_id" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors bg-white">
                                ...
                                @foreach($tasks as $task)
                                    <option value="{{ $task->id }}" {{ old('task_id', $materialRequest->task_id) == $task->id ? 'selected' : '' }}>{{ $task->name }} ({{ $task->task_code }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-2">
                            ...
                            <input type="date" name="request_date" id="request_date" required value="{{ old('request_date', $materialRequest->request_date->format('Y-m-d')) }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors">
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-2">
                            ...
                            <input type="date" name="required_date" id="required_date" value="{{ old('required_date', $materialRequest->required_date ? $materialRequest->required_date->format('Y-m-d') : '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors">
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-6">
                            ...
                            <textarea id="purpose" name="purpose" rows="3" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors">{{ old('purpose', $materialRequest->purpose) }}</textarea>
                                @error('purpose') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>

                        <div class="col-span-1 sm:col-span-2 lg:col-span-6">
                            <label for="notes" class="block text-sm font-medium text-gray-700 mb-1">Additional Notes</label>
                            <textarea id="notes" name="notes" rows="2" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-colors">{{ old('notes', $materialRequest->notes) }}</textarea>
            ...
            <div class="bg-white rounded-2xl border border-gray-200/60 shadow-sm overflow-hidden px-6 py-4 flex items-center justify-end gap-3 mb-8">
                <a href="{{ route('admin.material-requests.show', $materialRequest) }}" class="inline-flex items-center justify-center px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">Cancel</a>
    ...
    <div class="text-center py-16 bg-white rounded-2xl border border-gray-200/60 shadow-sm">
        ...
        <p class="text-sm text-gray-500 font-medium">You do not have permission to edit this material request.</p>
        <div class="mt-6">
            <a href="{{ route('admin.material-requests.show', $materialRequest) }}" class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-sm transition-all">Return to Request</a>
        </div>
    </div>
    @endcan
