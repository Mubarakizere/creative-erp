<x-layouts.admin title="Edit Material Request {{ $materialRequest->request_number }}">
    <x-slot:breadcrumbs>
        @php
            $breadcrumbs = [
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
                        Edit Material Request: {{ $materialRequest->request_number }}
                    </h1>
                    <p class="mt-1 text-sm text-slate-500 font-medium">Update details and items for this material request.</p>
                </div>
            </div>
            ...
            <form action="{{ route('admin.material-requests.update', $materialRequest) }}" method="POST" id="material-request-form">
                ...
                <x-card class="mb-6">
                    <x-slot:header>
                        ...
                        <p class="mt-0.5 text-sm text-slate-500">Update general information for this request.</p>
                    </x-slot:header>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6">
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-2">
                            <x-input type="date" name="request_date" label="Request Date" value="{{ old('request_date', $materialRequest->request_date->format('Y-m-d')) }}" required />
                        </div>

                        {{-- Required Date --}}
                        <div class="col-span-1 sm:col-span-2 lg:col-span-2">
                            <x-input type="date" name="required_date" label="Required Date" value="{{ old('required_date', $materialRequest->required_date ? $materialRequest->required_date->format('Y-m-d') : '') }}" />
                        </div>
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-6">
                            <x-textarea name="purpose" label="Purpose / Justification (Optional)" rows="3" placeholder="Explain why these materials are needed for the site work...">{{ old('purpose', $materialRequest->purpose) }}</x-textarea>
                        </div>

                        {{-- Additional Notes --}}
                        <div class="col-span-1 sm:col-span-2 lg:col-span-6">
                            <x-textarea name="notes" label="Additional Notes (Optional)" rows="2" placeholder="Any special delivery instructions or specifications...">{{ old('notes', $materialRequest->notes) }}</x-textarea>
                        </div>
                    </div>
                </x-card>
                    ...
                    <x-card class="mb-6">
                        <x-slot:header>
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="mt-0.5 text-sm text-slate-500 font-medium">Update products and quantities requested.</p>
                                </div>
                        ...
                        <x-slot:footer>
                            <div class="flex items-center justify-end gap-3 w-full">
                                <a href="{{ route('admin.material-requests.show', $materialRequest) }}" class="inline-flex justify-center items-center rounded-xl border border-slate-300 bg-white py-2.5 px-5 text-sm font-semibold text-slate-700 shadow-xs hover:bg-slate-50 focus:outline-none transition-all">Cancel</a>
    ...
    @else
        <div class="text-center py-16 bg-white rounded-2xl border border-slate-200 shadow-xs">
            ...
            <p class="text-sm text-slate-500 font-medium">You do not have permission to edit this material request.</p>
            <div class="mt-6">
                <a href="{{ route('admin.material-requests.show', $materialRequest) }}" class="px-5 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-xs transition-all">Return to Request</a>
            </div>
        </div>
    @endcan
