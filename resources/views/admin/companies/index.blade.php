        ...
        @forelse($companies as $company)
            <tr @class(['group', 'bg-red-50/30' => $company->trashed()])>
                ...
                <td class="px-4 py-3 text-right">
                    <x-action-dropdown>
                        @can('view', $company)
                            <x-action-dropdown-item href="{{ route('admin.companies.show', $company) }}" icon="view">
                                View Company
                        ...
                        @if(!$company->trashed())
                            @can('update', $company)
                                <x-action-dropdown-item href="{{ route('admin.companies.edit', $company) }}" icon="edit">
                                    Edit Company
                                </x-action-dropdown-item>
                            @endcan

                            @can('delete', $company)
                                <x-action-dropdown-item @click="$dispatch('open-modal', 'delete-company-{{ $company->id }}')" icon="delete" variant="danger">
                                    Delete Company
                                </x-action-dropdown-item>
                            @endcan
                        @else
                            @can('restore', $company)
                                <form method="POST" action="{{ route('admin.companies.restore', $company) }}" id="restore-company-form-{{ $company->id }}">
                                    @csrf
                                    @method('PATCH')
                                </form>
                                <x-action-dropdown-item onclick="document.getElementById('restore-company-form-{{ $company->id }}').submit()">
                                    Restore Company
                                </x-action-dropdown-item>
                            @endcan
                        @endif
