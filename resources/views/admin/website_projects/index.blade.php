<x-layouts.admin title="Website Projects">
    <x-slot:breadcrumbs>
        @php
            $breadcrumbs = [
                ['label' => 'Website CMS'],
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Website Projects</h1>
            <p class="mt-1 text-sm text-gray-500">Manage the dynamic cards displayed on the public Expertise page.</p>
        </div>
        <x-button type="primary" href="{{ route('admin.website-projects.create') }}">
                    ...
                    @forelse($projects as $project)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($project->image)
                                    <div class="h-12 w-20 rounded bg-gray-100 overflow-hidden border">
                                        <img src="{{ Str::startsWith($project->image, 'http') ? $project->image : asset($project->image) }}" class="h-full w-full object-cover">
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="text-xs text-gray-500 mt-1 line-clamp-1 max-w-xs">{{ $project->description }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                ...
                                <x-modal id="delete-card-{{ $project->id }}" maxWidth="md">
                                    ...
                                    <div class="text-center py-4">
                                        ...
                                        <h3 class="text-lg font-semibold text-gray-900 mb-2">Delete "{{ $project->title }}"?</h3>
